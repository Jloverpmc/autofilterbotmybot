...
from config import API_ID, API_HASH, BOT_TOKEN, DB_CHANNEL_ID, ADMIN_IDS, BOT_USERNAME, NOT_FOUND_MEDIA
...
async def group_search(client: Client, message: Message):
    ...
    if not files:
        if NOT_FOUND_MEDIA:
            await message.reply_photo(NOT_FOUND_MEDIA, caption="❌ Movie not available in the database.")
        else:
...
async def add_custom_button(client, message):
    # Usage: /addbutton <file_id> <Button Text> <URL>
